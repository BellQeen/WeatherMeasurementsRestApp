...
public class DrawChart {
    ...
    private static void drawChart(List<Double> temperatures) {
        double[] xData = IntStream.range(1, temperatures.size()+1).asDoubleStream().toArray();
        double[] yData = temperatures.stream().mapToDouble(x -> x).toArray();

        XYChart chart = QuickChart.getChart("Temperatures for the month", "Days", "Temperature, °C", "temperature",
                xData, yData);
